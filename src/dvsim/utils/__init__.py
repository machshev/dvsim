...
def _subst_wildcards(var, mdict, ignored, ignore_error, seen):
    ...
    while True:
        ...
        if name == "eval_cmd":
            cmd = _subst_wildcards(right_str[match.end() :], mdict, ignored, ignore_error, seen)[0]
...
def subst_wildcards(var, mdict, ignored_wildcards=None, ignore_error=False):
...
def find_and_substitute_wildcards(sub_dict, full_dict, ignored_wildcards=None, ignore_error=False):
...
def clean_odirs(odir, max_odirs, ts_format=TS_FORMAT):
    ...
    dirs = sorted(
        [old for old in pdir.iterdir() if (old.is_dir() and old.name != "summary")],
