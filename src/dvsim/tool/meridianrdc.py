    ...
    if m:
        ...
        for line in m[0].split("\n"):
            ...
            if re.match(r"^ GROUP\s+SDC_ENV_LINT", line):
                ...
                rule = re.findall(
                    r"^  INSTANCE\s+([a-zA-Z0-9\_]+)\s+([0-9\_]+)\s+(\d+)(\s+\d+)?",
                    line,
                )
                name = rule[0][0]
                ...
                if name in known_rule_names:
                    ...
                    rule_patterns.append((msg_group, rf"^{name}:.*"))
...
def main() -> None:
    ...
    rdc_rule_patterns = extract_rule_patterns(args.repdir.joinpath("REPORT/mrdc.new.rpt"))

    # Patterns for vcdc.rpt
    parser_args.update({args.repdir.joinpath("REPORT/mrdc.new.rpt"): rdc_rule_patterns})
