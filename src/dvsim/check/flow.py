...
def check_flow_config(
    hjson_path: Path, project_config: ProjectConfig | None = None
    ...
    Returns:
        Tuple of (success, message, flow_type, config):
        - success: True if config is valid, False otherwise
        - message: Human-readable message describing the result
        - flow_type: The detected flow type or None
        - config: The parsed config object or None if validation failed

    """
    hjson_path = Path(hjson_path)

    if not hjson_path.exists():
        return False, f"File not found: {hjson_path}", None, None
    ...
    if flow_type is None:
        return False, "No 'flow' field found in configuration", None, None

    # Check based on flow type
    if flow_type == "lint":
        try:
            config = parse_lint_flow_config(hjson_path, project_config)
        except (FileNotFoundError, RuntimeError) as e:
            return False, f"Invalid lint flow config: {e}", flow_type, None
        else:
            return True, f"Valid lint flow config: {config.name}", flow_type, config

    # Other flow types not yet supported
    return False, f"Flow type '{flow_type}' checking not yet implemented", flow_type, None
